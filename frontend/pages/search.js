...
const Search = () => {
    ...
    const handleSubmit = (event) => {
        ...
        console.log("検索経度緯度: ", lng, lat, '検索領域: ', path, "データ量: ", target_places.length)
    ...
    return (
        <div>
            ...
            <div>
                <ul>
                    {places.map(place => (
                        <li key={place.increment_id}> 施設名: {place.institution_name} & 経度: {place.longitude} & 緯度: {place.latitude} & 領域値: {place.path}</li>
                    ))}
                </ul>
            </div>
        </div>   
